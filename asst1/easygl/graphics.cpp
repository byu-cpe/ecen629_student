 ...
 * - Commented and reorganized some of the code.  Started cleaning up some of
 * the win32 code.  Win32 looks inefficient; it is saving and updating graphics
 * contexts all the time even though we know when the context is valid vs.
 * not-valid.
...
#ifndef NO_GRAPHICS // Strip everything out and just put in stubs if NO_GRAPHICS
                    // defined
...
#define SELECT_ERROR()                                                         \
  {                                                                            \
    char msg[BUFSIZE];                                                         \
    sprintf(msg,                                                               \
            "Error %i: Couldn't select graphics object on line %d of "         \
            "graphics.c\n",                                                    \
...
#define DELETE_ERROR()                                                         \
  {                                                                            \
    char msg[BUFSIZE];                                                         \
    sprintf(msg,                                                               \
            "Error %i: Couldn't delete graphics object on line %d of "         \
            "graphics.c\n",                                                    \
            GetLastError(), __LINE__);                                         \
    MessageBox(NULL, msg, NULL, MB_OK);                                        \
    exit(-1);                                                                  \
...
#define CREATE_ERROR()                                                         \
  {                                                                            \
    char msg[BUFSIZE];                                                         \
    sprintf(msg,                                                               \
            "Error %i: Couldn't create graphics object on line %d of "         \
            "graphics.c\n",                                                    \
            GetLastError(), __LINE__);                                         \
    MessageBox(NULL, msg, NULL, MB_OK);                                        \
    exit(-1);                                                                  \
...
/* Avoid funny clipping problems under windows that I suspect are caused by
 * round-off in the Win32 libraries.
 */
...
/* Used to define where the output of drawscreen (graphics primitives in the
 * user-controlled area) currently goes to: the screen or a postscript file.
 */
typedef enum { SCREEN = 0, POSTSCRIPT = 1 } t_display_type;

/* Indicates if this button displays text, a polygon or is just a separator.
 */
typedef enum { BUTTON_TEXT = 0, BUTTON_POLY, BUTTON_SEPARATOR } t_button_type;

/* Structure used to define the buttons on the right hand side of the main
 * window (menu region). width, height: button size, in pixels. xleft, ytop:
 * coordinates, in pixels, of the top-left corner of the button relative to its
 *    containing (menu) window.
 * fcn: a callback function that is called when the button is pressed. This
 * function takes one argument, a function pointer to the routine that can draw
 * the graphics area (user routine). win, hwnd:  X11 and Win32 data pointer to
 * the window, respectively. button_type: indicates if this button displays
 * text, a polygon or is just a separator. text: the text to display if this is
 * a text button. poly: the polygon (up to 3 points right now) to display if
 * this is a polygon button is_pressed: has the button been pressed, and is
 * currently executing its callback? is_enabled: can you press this button right
 * now? Visually will look "pushed in" when not enabled, and won't respond to
 * clicks.
 */
typedef struct {
  int width;
...
static float ps_left, ps_right, ps_top,
    ps_bot;                      /* Figure boundaries for *
...
static COLORREF win32_colors[NUM_COLOR] = {
    RGB(255, 255, 255), RGB(0, 0, 0),   RGB(128, 128, 128), RGB(192, 192, 192),
    RGB(0, 0, 255),     RGB(0, 255, 0), RGB(255, 255, 0),   RGB(0, 255, 255),
    RGB(255, 0, 0),     RGB(0, 128, 0), RGB(255, 0, 255)};
...
/* These are used for the "Window" graphics button. They keep track of whether
 * we're entering the window rectangle to zoom to, etc.
 */
 ...
 */
static LRESULT CALLBACK GraphicsWND(HWND hwnd, UINT message, WPARAM wParam,
                                    LPARAM lParam);
static LRESULT CALLBACK StatusWND(HWND hwnd, UINT message, WPARAM wParam,
                                  LPARAM lParam);
static LRESULT CALLBACK ButtonsWND(HWND hwnd, UINT message, WPARAM wParam,
                                   LPARAM lParam);
static LRESULT CALLBACK MainWND(HWND hwnd, UINT message, WPARAM wParam,
                                LPARAM lParam);
...
/* Sets the current graphics context colour to cindex, regardless of whether we
 * think it is needed or not.
 */
static void force_setcolor(int cindex) {
  static const char *ps_cnames[NUM_COLOR] = {
      "white",  "black", "grey55", "grey75",    "blue",   "green",
      "yellow", "cyan",  "red",    "darkgreen", "magenta"};
  ...
  if (disp_type == SCREEN) {
    ...
    if (!DeleteObject(hGraphicsPen))
      DELETE_ERROR();
    hGraphicsPen = ExtCreatePen(PS_GEOMETRIC | win_linestyle | PS_ENDCAP_FLAT,
                                currentlinewidth, &lb, (LONG)NULL, NULL);
    if (!hGraphicsPen)
      CREATE_ERROR();
...
/* Sets the current graphics context colour to cindex if it differs from the old
 * colour */
...
/* Change the linestyle in the graphics context only if it differs from the
 * current linestyle.
 */
...
/* Sets the linewidth in the grahpics context, if it differs from the current
 * value.
 */
...
#ifdef X11
    button[bnum].win = XCreateSimpleWindow(
        display, menu, button[bnum].xleft, button[bnum].ytop,
        button[bnum].width, button[bnum].height, 0, colors[WHITE],
        colors[LIGHTGREY]);
    XMapWindow(display, button[bnum].win);
    XSelectInput(display, button[bnum].win, ButtonPressMask);
#else
    button[bnum].hwnd =
        CreateWindow(TEXT("button"), TEXT(button[bnum].text),
                     WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, button[bnum].xleft,
                     button[bnum].ytop, button[bnum].width, button[bnum].height,
                     hButtonsWnd, (HMENU)(200 + bnum),
...
void init_graphics(const char *window_name, int cindex) {
  ...
  const char *cnames[NUM_COLOR] = {
      "white",  "black", "grey55", "grey75",           "blue",   "green",
      "yellow", "cyan",  "red",    "RGBi:0.0/0.5/0.0", "magenta"};
  ...
  toplevel = XCreateSimpleWindow(display, RootWindow(display, screen_num), x, y,
                                 top_width, top_height, border_width,
                                 colors[BLACK], colors[cindex]);
  ...
  gcxor = XCreateGC(display, toplevel, (GCFunction | GCForeground), &values);
  ...
  hGraphicsPen = ExtCreatePen(
      PS_GEOMETRIC | win32_line_styles[currentlinestyle] | PS_ENDCAP_FLAT, 1,
      &lb, (LONG)NULL, NULL);
  if (!hGraphicsPen)
    CREATE_ERROR();
  ...
  if (!RegisterClass(&wndclass)) {
    MessageBox(
        NULL,
        TEXT("Initialization of Windows graphics (init_graphics) failed."),
  ...
  hMainWnd =
      CreateWindow(szAppName, TEXT(window_name), WS_OVERLAPPEDWINDOW, x, y,
                   top_width, top_height, NULL, NULL, hInstance, NULL);
...
#endif
      if (get_mouse_move_input && report.xmotion.x <= top_width - MWIDTH &&
          report.xmotion.y <= top_height - T_AREA_HEIGHT)
        act_on_mousemove(XTOWORLD(report.xmotion.x),
                         YTOWORLD(report.xmotion.y));
      break;
    case KeyPress:
#ifdef VERBOSE
...
void drawline(float x1, float y1, float x2, float y2) {
  ...
  if (disp_type == SCREEN) {
    ...
    XDrawLine(display, toplevel, current_gc, xcoord(x1), ycoord(y1), xcoord(x2),
              ycoord(y2));
#else /* Win32 */
...
void drawrect(float x1, float y1, float x2, float y2) {
  ...
  if (disp_type == SCREEN) {
    ...
    if (!(hOldPen = (HPEN)SelectObject(hGraphicsDC, hGraphicsPen)))
      SELECT_ERROR();
    if (!(hOldBrush =
              (HBRUSH)SelectObject(hGraphicsDC, GetStockObject(NULL_BRUSH))))
      SELECT_ERROR();
...
void drawellipticarc(float xc, float yc, float radx, float rady, float startang,
                     float angextent) {
  int xl, yt;
  unsigned int width, height;
#ifdef WIN32
  ...
  if (disp_type == SCREEN) {
    ...
    if (angextent > 0) {
      ...
      p3 = (int)(xcoord(xc) + fabs(xmult * radx) *
                                  cos(DEGTORAD(startang + angextent - .001)));
      p4 = (int)(ycoord(yc) - fabs(ymult * rady) *
                                  sin(DEGTORAD(startang + angextent - .001)));
    } else {
      p1 = (int)(xcoord(xc) + fabs(xmult * radx) *
                                  cos(DEGTORAD(startang + angextent + .001)));
      p2 = (int)(ycoord(yc) - fabs(ymult * rady) *
                                  sin(DEGTORAD(startang + angextent + .001)));
...
/* Startang is relative to the Window's positive x direction.  Angles in
 * degrees.
 */
 ...
 */
void fillellipticarc(float xc, float yc, float radx, float rady, float startang,
                     float angextent) {
  int xl, yt;
  unsigned int width, height;
#ifdef WIN32
  ...
  if (disp_type == SCREEN) {
    ...
    if (!(hOldBrush = (HBRUSH)SelectObject(hGraphicsDC, hGraphicsBrush)))
      SELECT_ERROR();
    // Win32 API says a zero return value indicates an error, but it seems to
    // always return zero.  Don't check for an error on Pie.
    Pie(hGraphicsDC, xl, yt, xl + width, yt + height, p1, p2, p3, p4);

    //		if(!Pie(hGraphicsDC, xl, yt, xl+width, yt+height, p1, p2, p3,
    // p4)); 			DRAW_ERROR();
    if (!SelectObject(hGraphicsDC, hOldPen))
      SELECT_ERROR();
    if (!SelectObject(hGraphicsDC, hOldBrush))
...
void drawtext(float xc, float yc, const char *text, float boundx) {
  ...
  if (disp_type == SCREEN) {
#ifdef X11
    XDrawString(display, toplevel, current_gc, xcoord(xc) - width / 2,
    ...
    if (!TextOut(hGraphicsDC, xcoord(xc) - width / 2,
                 ycoord(yc) - (font_ascent + font_descent) / 2, text, len))
      DRAW_ERROR();
    if (!SelectObject(hGraphicsDC, hOldFont))
      SELECT_ERROR();
#endif
...
void draw_message(void) {
  ...
  if (disp_type == SCREEN) {
    ...
    XDrawRectangle(display, textarea, gc_menus, 0, 0, top_width - MWIDTH,
                   T_AREA_HEIGHT);
    XSetForeground(display, gc_menus, colors[BLACK]);
    XDrawLine(display, textarea, gc_menus, 0, T_AREA_HEIGHT - 1,
              top_width - MWIDTH, T_AREA_HEIGHT - 1);
    XDrawLine(display, textarea, gc_menus, top_width - MWIDTH - 1, 0,
              top_width - MWIDTH - 1, T_AREA_HEIGHT - 1);

    XDrawString(display, textarea, gc_menus, (top_width - MWIDTH - width) / 2,
                T_AREA_HEIGHT / 2 + (font_info[menu_font_size]->ascent -
                                     font_info[menu_font_size]->descent) /
                                        2,
                statusMessage, len);
#else
...
static void proceed(void (*drawscreen)(void)) { ProceedPressed = TRUE; }
...
int init_postscript(const char *fname) {
  ...
  fprintf(ps, "   yoff rmoveto         %% Move left that much and down half "
              "font height\n");
...
static void build_default_menu(void) {
  ...
  const int NUM_ARROW_BUTTONS = 4, NUM_STANDARD_BUTTONS = 12,
            SEPARATOR_BUTTON_INDEX = 8;
  ...
  menu =
      XCreateSimpleWindow(display, toplevel, top_width - MWIDTH, 0, MWIDTH,
                          display_height, 0, colors[BLACK], colors[LIGHTGREY]);
...
#define NUM_FONT_TYPES 2
  ...
  for (ifont = 0; ifont < NUM_FONT_TYPES; ifont++) {
    ...
    if ((font_info[pointsize] = XLoadQueryFont(display, fontname[ifont])) ==
        NULL) {
#ifdef VERBOSE
...
void set_mouse_move_input(bool enable) { get_mouse_move_input = enable; }

void set_keypress_input(bool enable) { get_keypress_input = enable; }
...
static void build_textarea(void) {
  ...
  textarea = XCreateSimpleWindow(
      display, toplevel, 0, top_height - T_AREA_HEIGHT, display_width - MWIDTH,
...
static void menutext(Window win, int xc, int yc, const char *text) {
  ...
  XDrawString(display, win, gc_menus, xc - width / 2,
              yc + (font_info[menu_font_size]->ascent -
                    font_info[menu_font_size]->descent) /
                       2,
              text, len);
}
...
static void drawbut(int bnum) {
  ...
  if (button[bnum].type == BUTTON_TEXT) {
    if (button[bnum].enabled)
      ...
      XSetForeground(display, gc_menus, colors[DARKGREY]);
    menutext(button[bnum].win, button[bnum].width / 2, button[bnum].height / 2,
             button[bnum].text);
  }
}
...
static LRESULT CALLBACK MainWND(HWND hwnd, UINT message, WPARAM wParam,
  ...
  case WM_CREATE:
    hStatusWnd = CreateWindow(
        szStatusName, NULL, WS_CHILDWINDOW | WS_VISIBLE, 0, 0, 0, 0, hwnd,
        (HMENU)102, (HINSTANCE)GetWindowLong(hwnd, GWL_HINSTANCE), NULL);
    hButtonsWnd = CreateWindow(
        szButtonsName, NULL, WS_CHILDWINDOW | WS_VISIBLE, 0, 0, 0, 0, hwnd,
        (HMENU)103, (HINSTANCE)GetWindowLong(hwnd, GWL_HINSTANCE), NULL);
    hGraphicsWnd = CreateWindow(
        szGraphicsName, NULL, WS_CHILDWINDOW | WS_VISIBLE, 0, 0, 0, 0, hwnd,
        (HMENU)101, (HINSTANCE)GetWindowLong(hwnd, GWL_HINSTANCE), NULL);
    return 0;
  ...
  case WM_SIZE:
    ...
    if (!MoveWindow(hGraphicsWnd, 1, 1, top_width - MWIDTH - 1,
                    top_height - T_AREA_HEIGHT - 1, TRUE))
      DRAW_ERROR();
    //		if (drawscreen_ptr)
    //			zoom_fit(drawscreen_ptr);
    if (!MoveWindow(hStatusWnd, 0, top_height - T_AREA_HEIGHT,
                    top_width - MWIDTH, T_AREA_HEIGHT, TRUE))
      DRAW_ERROR();
    if (!MoveWindow(hButtonsWnd, top_width - MWIDTH, 0, MWIDTH, top_height,
                    TRUE))
      DRAW_ERROR();
...
static LRESULT CALLBACK GraphicsWND(HWND hwnd, UINT message, WPARAM wParam,
  ...
  case WM_CREATE:
    ...
    if (!SetMapMode(hBackgroundDC, MM_TEXT))
      CREATE_ERROR();
    if (!(hbmBuffer = CreateCompatibleBitmap(hForegroundDC, display_width,
                                             display_height)))
      CREATE_ERROR();
    ...
    if (!SetMapMode(hObjtestDC, MM_TEXT))
      CREATE_ERROR();
    if (!(hbmObjtest = CreateCompatibleBitmap(hObjtestDC, display_width,
                                              display_height)))
      CREATE_ERROR();
    ...
    if (!SetMapMode(hAllObjtestDC, MM_TEXT))
      CREATE_ERROR();
    if (!(hbmAllObjtest = CreateCompatibleBitmap(hAllObjtestDC, display_width,
                                                 display_height)))
      CREATE_ERROR();
    ...
    if (InEventLoop) {
      ...
      if (windowAdjustFlag > 1) {
        ...
        if (!Rectangle(hGraphicsDC, oldAdjustRect.left, oldAdjustRect.top,
          ...
          DRAW_ERROR();
        if (!Rectangle(hGraphicsDC, adjustRect.left, adjustRect.top,
                       adjustRect.right, adjustRect.bottom))
          DRAW_ERROR();
  ...
  case WM_LBUTTONDOWN:
    if (!windowAdjustFlag) {
      ...
      // First you push the button, then you click for one corner, then you
      // click for the other corner.
      if (windowAdjustFlag == 1) {
        windowAdjustFlag++;
...
static LRESULT CALLBACK StatusWND(HWND hwnd, UINT message, WPARAM wParam,
  ...
  case WM_PAINT:
    ...
    if (!DrawText(hdc, TEXT(statusMessage), -1, &rect,
...
void drawtobuffer(void) { hGraphicsDC = hBackgroundDC; }

void drawtoscreen(void) { hGraphicsDC = hForegroundDC; }

void displaybuffer(void) {
  if (!BitBlt(hForegroundDC, xcoord(xleft), ycoord(ytop),
              xcoord(xright) - xcoord(xleft), ycoord(ybot) - ycoord(ytop),
              hBackgroundDC, // hAllObjtestDC,
              0, 0, SRCCOPY))
    DRAW_ERROR();
}
...
void setcolor_by_colorref(COLORREF c) {
  ...
  if (!DeleteObject(hGraphicsPen))
    DELETE_ERROR();
  hGraphicsPen = ExtCreatePen(
      PS_GEOMETRIC | win32_line_styles[currentlinestyle] | PS_ENDCAP_FLAT,
      currentlinewidth, &lb, (LONG)NULL, NULL);
  if (!hGraphicsPen)
    CREATE_ERROR();
...
static void _drawcurve(t_point *points, int npoints, int fill) {
  ...
  if (disp_type == SCREEN) {
    ...
    if (!fill) {
      ...
      if (!(hOldPen =
                (HPEN)SelectObject(hGraphicsDC, GetStockObject(NULL_PEN))))
        SELECT_ERROR();
      if (!(hOldBrush = (HBRUSH)SelectObject(hGraphicsDC, hGraphicsBrush)))
        SELECT_ERROR();
    }
    ...
    for (i = 1; i < npoints; i += 3)
      fprintf(ps, "%.2f %.2f %.2f %.2f %.2f %.2f curveto\n", XPOST(points[i].x),
              YPOST(points[i].y), XPOST(points[i + 1].x),
              YPOST(points[i + 1].y), XPOST(points[i + 2].x),
              YPOST(points[i + 2].y));
    if (!fill)
      fprintf(ps, "stroke\n");
    else
...
void drawcurve(t_point *points, int npoints) { _drawcurve(points, npoints, 0); }

void fillcurve(t_point *points, int npoints) { _drawcurve(points, npoints, 1); }
...
void object_end() { hGraphicsDC = hCurrentDC; }
...
static int check_fontsize(int pointsize, float ymax) {
...
#else /***** NO_GRAPHICS *******/
/* No graphics at all. Stub everything out so calling program doesn't have to
 * change but of course graphics won't do anything.
 */
...
int init_postscript(const char *fname) { return (1); }
